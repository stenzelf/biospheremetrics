...
#' @param cell_area array with area for each cell of internal indicator
...
calc_roc_data <- function(external_binary, # binary transgression: 0 - no,1 - yes
  ...
  for (i in seq_along(potential_thresholds)) {
    ...
    internal_cells <- which(internal_continuous > t) # nolint
    TP <- sum(cell_area[which(external_binary == 1 & internal_continuous > t)]) # True Positive
    FP <- sum(cell_area[which(!(external_binary == 1) & internal_continuous > t)]) # False Positive
    values[i, 1, 1] <- TP
    values[i, 1, 2] <- FP
  }
...
calc_roc_data_3d <- function(external_continuous,
  ...
  for (e in seq_along(potential_thresholds_internal)) {
    for (i in seq_along(potential_thresholds_internal)) {
      ...
      TP <- sum(cell_area[which(external_continuous > te & internal_continuous > ti)]) # True Positive
      FP <- sum(cell_area[which(external_continuous <= te & internal_continuous > ti)]) # False Positive
      values[i, e, 1] <- TP
      values[i, e, 2] <- FP
    }
  }
...
roc_plot <- function(filename = NULL,
  ...
  graphics::par(oma = c(0, 0, 0, 0), mar = c(6, 10, 5, 0), bty = "o", xpd = FALSE, xaxs = "i", yaxs = "i") # nolint
  ...
  for (ind in indizes) {
    ...
    best_max_diff <- sort(abs(values[, ind, 2] - values[, ind, 1]), decreasing = TRUE)[1:5]
    best_slope <- sort(abs(values[, ind, 3] - 1), decreasing = FALSE)[1:5]
    closest <- function(x, y) {
      abs((x - y) / sqrt(2))
    }
    best_max_diff_thr <- names(sort(closest(x = values[names(best_max_diff), ind, 2],
                                            y = values[names(best_max_diff), ind, 1]), decreasing = TRUE)[1])
    best_slope_thr <- names(sort(closest(x = values[names(best_slope), ind, 2],
                                         y = values[names(best_slope), ind, 1]), decreasing = TRUE)[1])
...
roc_plot_paper <- function(filename = NULL,
  ...
  for (var in vars) {
    ...
    for (ind in indizes) {
      ...
      best_max_diff <- sort(abs(values[, var, ind, 2] - values[, var, ind, 1]), decreasing = TRUE)[1:5]
      best_slope <- sort(abs(values[, var, ind, 3] - 1), decreasing = FALSE)[1:5]
      closest <- function(x, y) {
        abs((x - y) / sqrt(2))
      }
      best_max_diff_thr <- names(sort(closest(x = values[names(best_max_diff), var, ind, 2],
                                              y = values[names(best_max_diff), var, ind, 1]), decreasing = TRUE)[1])
      best_slope_thr <- names(sort(closest(x = values[names(best_slope), var, ind, 2],
                                           y = values[names(best_slope), var, ind, 1]), decreasing = TRUE)[1])
  ...
  graphics::text(x = 1, y = max(biocol_threshold$stats) + 0.2,
  ...
  graphics::points(x = rep(0.7, length(indizes)), y = as.numeric(best_fit[indizes, "EcoRisk", 1]),
                   col = scales::alpha(colz[indizes], 1), lwd = 2)
  graphics::text(x = 1, y = max(ecorisk_threshold$stats) + 0.2, 
                 paste0("median:\n", stats::median(ecorisk_threshold$stats)), cex = cex_perc)
  graphics::mtext(side = 3, at = 0, "D", xpd = NA, cex = 1.5, font = 2)

  # AUC boxplots
  graphics::boxplot(auc[indizes, "BioCol"], ylim = c(0, 1), main = "AUC", cex.main = cex_perc,
  ...
  graphics::boxplot(auc[indizes, "EcoRisk"], main = "AUC", cex.main = cex_perc,
                    ylim = c(0, 1), cex.lab = cex_perc, cex.axis = cex_perc)
